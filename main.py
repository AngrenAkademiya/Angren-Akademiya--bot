...
def save_to_excel(data):
    ...
    ws.append([
        tartib_raqam, sana, data.get("name"), data.get("phone"), data.get("parent_phone"),
        data.get("school"), data.get("grade"), data.get("filial"), data.get("time_pref"),
        courses_string, data.get("user_id")
    ])
    ...
    try:
        ...
        logging.warning(f"Excel formatlashda xato: {e}")
    wb.save(EXCEL_FILE)
...
def _write_to_google_sheets_sync(data):
    ...
    try:
        ...
        sheet.append_row(["№", "Sana", "Ism Familiya", "Tel Raqam", "Ota-ona Tel", "Maktab", "Sinf", "Filial", "Smena", "Kurslar", "ID"])
...
async def save_to_google_sheets(data):
    try:
        ...
        if admin_id:
            try:
                await bot.send_message(int(admin_id), f"Google Sheets xato: {type(e).name}: {e}")
            except Exception:
                pass
...
AVAILABLE_SUBJECTS = [
    "Matematika - Milliy va xalqaro sertifikat", "Matematika - majburiy blok uchun",
    "Ingliz tili - IELTS", "Tibbiyot - shifokorlik kasblari uchun\nKimyo - Milliy va xalqaro sertifikat",
    "Prezident maktablariga tayyorlov", "Al-Xorazmiy maktablariga tayyorlov",
    "Tibbiyot - shifokorlik kasbini tanlaganlar uchun\nBiologiya - Milliy va xalqaro sertifikat",
    "Tarix - Milliy sertifikat", "Tarix - Majburiy blok uchun", "Huquq - Milliy sertifikat",
    "IT - Milliy va xalqaro sertifikat", "Ona tili va adabiyoti - Milliy sertifikat",
    "Ona tili va adabiyoti - Majburiy blok uchun", "Maktabga tayyorlov. Pochemuchka"
]
...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("✨ Angren Akademiyasi rasmiy botiga xush kelibsiz!\n\nKelajak akademiyasida o'z bilimingizni va farzandingiz kamolotini nazorat qiling.", reply_markup=get_main_menu())

@dp.message(F.text == "/excel")
async def send_excel(message: types.Message):
    if os.path.exists(EXCEL_FILE):
        await message.answer_document(document=FSInputFile(EXCEL_FILE), caption="📊 Angren Akademiya o'quvchilar ro'yxati (Excel)")
    else:
...
async def process_grade(message: types.Message, state: FSMContext):
    ...
    kb = ReplyKeyboardBuilder()
    for filial in AVAILABLE_FILIALS: kb.button(text=filial)
    kb.adjust(2)
...
async def show_subjects_keyboard(message, selected_courses):
    ...
    if selected_courses: text += "Tanlanganlar:\n" + "\n".join(f"- {c.replace(chr(10), ' ')}" for c in selected_courses)
    if isinstance(message, types.Message): await message.answer(text, reply_markup=kb.as_markup())
    else: await message.message.edit_text(text, reply_markup=kb.as_markup())
...
async def process_subjects(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "done":
        if not selected_courses:
            ...
            return
        kb = ReplyKeyboardBuilder()
        for t in AVAILABLE_TIMES: kb.button(text=t)
        kb.adjust(3)
    ...
    if subject_name in selected_courses: selected_courses.remove(subject_name)
    else: selected_courses.append(subject_name)
...
def escape_markdown(text):
    ...
    text = str(text)
    for ch in ("_", "*", "`", "["): text = text.replace(ch, "\\" + ch)
    return text
...
async def process_time_pref(message: types.Message, state: FSMContext):
    if message.text not in AVAILABLE_TIMES:
        ...
        return
    await state.update_data(time_pref=message.text, user_id=message.from_user.id)
...
student_report = (
    ...
    if is_medical:
        try:
            await message.answer_photo(photo=FSInputFile("IMG_20260619_235730_628.jpg"), caption=student_report, parse_mode="Markdown", reply_markup=get_main_menu())
        except Exception:
...
async def handle_health(request): return web.Response(text="Angren Akademiya boti faol!")
